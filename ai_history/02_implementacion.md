...
**Objetivo:** Evitar que archivos temporales y compilados ensucien el �rbol de trabajo y los commits.

### Tareas realizadas
- Se cre� el archivo .gitignore en la ra�z del proyecto.
- Se configur� para ignorar directorios de cach� de Python (__pycache__/, *.pyc, *.pyo, *.pyd).
- Se configur� para ignorar bases de datos SQLite (*.db, *.sqlite, *.sqlite3).
- Se configur� para ignorar entornos virtuales (.venv/, env/).
- Se configur� para ignorar archivos de variables de entorno y del SO (.env, .DS_Store, Thumbs.db).
- Se valid� con git status que los directorios generados por pytest (src/__pycache__ y 	ests/__pycache__) fueron autom�ticamente excluidos del rastreo.
- Se gener� el commit (chore: add .gitignore for python, sqlite and OS artifacts) para consolidar la limpieza del repositorio.
